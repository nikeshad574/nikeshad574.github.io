import { Link } from "react-router";
import ProjectCard from "../../components/ProjectCard";
import { ChevronRightCircle, Globe } from "lucide-react";
import cns from "../../utils/classNames";

function SingleProject() {
  return (
    <section className="container flex gap-4">
      <div className=" w-full p-2">
        <div
          className={cns(
            "aspect-video w-full bg-primary-800 rounded-xl relative mb-2"
          )}
        >
          <img
            src="/test.png"
            alt=""
            className="h-full rounded-xl w-full object-cover"
          />
          <div className=" bg-primary-900 pt-2 pl-2 rounded-tl-xl absolute right-0 bottom-0">
            <div className="flex items-center justify-center gap-2 p-2 bg-primary-800 rounded-xl  ring-primary-900">
              <Link
                to="/projects"
                className="h-8 w-8 p-0.5 hover:text-primary-400"
                title="View Project Code"
              >
                <svg
                  role="img"
                  viewBox="0 0 24 24"
                  xmlns="http://www.w3.org/2000/svg"
                  fill="currentColor"
                >
                  <title>GitHub</title>
                  <path d="M12 .297c-6.63 0-12 5.373-12 12 0 5.303 3.438 9.8 8.205 11.385.6.113.82-.258.82-.577 0-.285-.01-1.04-.015-2.04-3.338.724-4.042-1.61-4.042-1.61C4.422 18.07 3.633 17.7 3.633 17.7c-1.087-.744.084-.729.084-.729 1.205.084 1.838 1.236 1.838 1.236 1.07 1.835 2.809 1.305 3.495.998.108-.776.417-1.305.76-1.605-2.665-.3-5.466-1.332-5.466-5.93 0-1.31.465-2.38 1.235-3.22-.135-.303-.54-1.523.105-3.176 0 0 1.005-.322 3.3 1.23.96-.267 1.98-.399 3-.405 1.02.006 2.04.138 3 .405 2.28-1.552 3.285-1.23 3.285-1.23.645 1.653.24 2.873.12 3.176.765.84 1.23 1.91 1.23 3.22 0 4.61-2.805 5.625-5.475 5.92.42.36.81 1.096.81 2.22 0 1.606-.015 2.896-.015 3.286 0 .315.21.69.825.57C20.565 22.092 24 17.592 24 12.297c0-6.627-5.373-12-12-12" />
                </svg>
              </Link>

              <Link
                to="/"
                className="h-8 w-8 p-0.5 hover:text-primary-400"
                title="View Live Project"
              >
                <Globe />
              </Link>

              <Link
                to="/"
                className="h-8 w-8 p-0.5 hover:text-primary-400"
                title="Read More"
              >
                <ChevronRightCircle />
              </Link>
            </div>
          </div>
        </div>

        <h1 className="text-2xl font-medium">
          Lorem ipsum dolor sit amet consectetur adipisicing elit. Id minus
          culpa laboriosam, ipsam optio totam quo doloremque dolore harum
          ducimus nobis perspiciatis. Provident, assumenda?
        </h1>

        <div className="flex flex-wrap items-center gap-2 mt-2">
          {Array.from({ length: 5 }).map((_, idx) => (
            <Link
              to="/"
              className="px-2 py-1 rounded-lg bg-primary/20 hover:bg-primary"
              key={idx}
            >
              Skills
            </Link>
          ))}
        </div>

        <div className="htmlContent flex flex-col gap-2 mt-4">
          <p>
            Lorem ipsum dolor sit, amet consectetur adipisicing elit. Aperiam
            dolores veritatis commodi exercitationem deserunt saepe fugiat quod
            dolorum, placeat natus reiciendis consequatur maxime eos, fugit
            neque impedit voluptates itaque ea.
          </p>

          <h1>h1</h1>
          <h2>h2</h2>
          <h3>h3</h3>
          <h4>h4</h4>
          <h5>h5</h5>

          <ul>
            <li>One</li>
            <li>One</li>
            <ul>
              <li>two</li>
              <li>two</li>
              <li>two</li>
              <li>two</li>
            </ul>
            <li>One</li>
          </ul>

          <ol>
            <li>lsdksld</li>
            <li>lsdksld</li>
            <li>lsdksld</li>
            <li>lsdksld</li>
          </ol>

          <p>
            Lorem ipsum dolor sit amet consectetur adipisicing elit. Pariatur,
            numquam.
          </p>

          <img src="/test.png" alt="" />

          <p>
            Lorem ipsum dolor sit amet consectetur adipisicing elit. Atque,
            velit?
          </p>

          <table>
            <thead>
              <th>One</th>
              <th>One</th>
              <th>One</th>
              <th>One</th>
            </thead>
            <tbody>
              <tr>
                <td>two</td>
                <td>two</td>
                <td>two</td>
                <td>two</td>
              </tr>
            </tbody>
          </table>

          <code>hello Nepali How are you</code>
          <pre>hello Nepali How are you</pre>

          <a href="#">Hello Nepali</a>
        </div>
      </div>

      <div className="w-full max-w-80 flex flex-col gap-4 p-2">
        <h2 className="text-xl font-medium">More Projects</h2>

        <ProjectCard />
        <ProjectCard />
        <ProjectCard />
      </div>
    </section>
  );
}

export default SingleProject;
